...
public class GameSocket {
	...
	public void onTextMessage(Session session, String msg, boolean last) {
		try {
			if (session.isOpen()) {
				logger.error("received from client message = " + msg);
				...
				*/
				...
				switch(mess[0]){
					case "login":
						...
						logger.error("user number connect: "+sess2user.size());
						if(u1.passmatch())
    ...
    public void echoBinaryMessage(Session session, ByteBuffer bb, boolean last) {
    	//logger.error("send binary message...");
        try {
            if (session.isOpen()) {
            	//logger.error("byte buffer lenghth : " + bb.array().length);
    ...
    public void onclose(Session sess){
    	...
    	try{
    		...
    		if(!sess2user.remove(sess, u))
    			logger.error("cannot rm map");
    		broadcast_putong();
    		logger.error("user number disconnect: "+sess2user.size());
    	}finally{
